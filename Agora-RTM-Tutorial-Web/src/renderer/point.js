...
Point.prototype.parsePoints = function (data) {
    ...
    for(var index=0; index<pointNum; index++){
        xl = buff.getUint8(0 + (index*this.POINT_LEN_BYTE));
        xh = buff.getUint8(1 + (index*this.POINT_LEN_BYTE));
        yl = buff.getUint8(2 + (index*this.POINT_LEN_BYTE));
        yh = buff.getUint8(3 + (index*this.POINT_LEN_BYTE));
        pl = buff.getUint8(4 + (index*this.POINT_LEN_BYTE));
        ph = buff.getUint8(5 + (index*this.POINT_LEN_BYTE));
        X = ((xh<<8)&0xff00) + xl;
        Y = ((yh<<8)&0xff00) + yl;
        P = ((ph<<8)&0xff00) + pl;
